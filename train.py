...
def main():
    ...
    parser.add_argument('-opt', type=str, default = '/home/jili/HTC_code/confs/sino_conf.yml', help='Path to option YMAL file.')
...
# print the dataset
    ...
    #     x = train_data['fbp'].detach()[0,0].float().cpu().numpy()
    #     y = (x - np.min(x))/(np.max(x)-np.min(x))

    #     # sr_img = util.tensor2img()  # uint8
    #     save_img_path = os.path.join(img_dir,
    #                                     '{:d}_fbp.png'.format(i))
    #     imsave(save_img_path,y)
    #     x = train_data['label'].detach()[0,0].float().cpu().numpy()
    #     y = (x - np.min(x))/(np.max(x)-np.min(x))
    #     save_img_path = os.path.join(img_dir,
    #                                     '{:d}_label.png'.format(i))
    #     imsave(save_img_path,y)
    ...
    # load the model
    # model.load_network(load_path='/home/jili/HTC_code/experiments/train_ang70_shape/models/49000_G.pth', network=model.netG)
    #### training
    timer = Timer()
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            try:
                nll, losses = model.optimize_parameters(current_step)
            ...
            if current_step % opt['logger']['print_freq'] == 0 \
                ...
                # message = '<epoch:{:3d}, iter:{:8,d}, lr:{:.3e}, t:{:.2e}, td:{:.2e}, eta:{:.2e}, nll:{:.3e}, d_l:{:.3e}, reg_1:{:.3e}, reg_2:{:.3e}> '.format(
                #     epoch, current_step, model.get_current_learning_rate(), avg_time, avg_data_time,
                #     eta(avg_time), nll, losses['x'].item(),losses['reg_1'].item(),losses['reg_2'].item())
                message = '<epoch:{:3d}, iter:{:8,d}, lr:{:.3e}, t:{:.2e}, td:{:.2e}, eta:{:.2e}, nll:{:.3e}, d_l:{:.3e}, reg_1:{:.3e}> '.format(
                    epoch, current_step, model.get_current_learning_rate(), avg_time, avg_data_time,
                    eta(avg_time), nll, losses['x'].item(),0.0)
                print(message)
            timer.tick()
            ...
            if current_step % opt['train']['val_freq'] == 0 and rank <= 0:
                ...
                visuals = model.get_current_visuals()
                x = visuals['fbp'].numpy().squeeze()
                inp = (x - np.min(x))/(np.max(x)-np.min(x))

                # sr_img = util.tensor2img()  # uint8
                save_img_path = os.path.join(img_dir,
                                                '{:s}_{:d}_fbp.png'.format(img_name, current_step))
                imsave(save_img_path,inp)
                x = visuals['label'].numpy().squeeze()
                y = (x - np.min(x))/(np.max(x)-np.min(x))
                save_img_path = os.path.join(img_dir,
                                                '{:s}_{:d}_label.png'.format(img_name, current_step))
                imsave(save_img_path,y)

                x = visuals['fake_H'].numpy().squeeze()
                rec = (x - np.min(x))/(np.max(x)-np.min(x))
                save_img_path = os.path.join(img_dir,
                                                '{:s}_{:d}_rec.png'.format(img_name, current_step))
                imsave(save_img_path,rec)
                

                # log
                avg_psnr = np.mean((rec-y)**2)

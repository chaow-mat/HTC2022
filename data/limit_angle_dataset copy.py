...
class CTDataset(Dataset):
    def __init__(self, image_dir, angle = None, transform=None, is_test=False):
        self.sino_dir = image_dir # /home/jili/HTC2022/
        self.no_angle = angle
    ...
    def __getitem__(self, idx):
        ...
        alpha_start = 30 #self.start_angle[idx]
        # else:
        ...
        # if self.transform:
        #     fbp, label = self.transform(fbp, label)
        # else:
        #     fbp = F.to_tensor(fbp)
        #     label = F.to_tensor(label)
        ...
        # return image, label
        # print(image)
        return {'Sino': sinogram_trans, 'fbp': torch.cat([fbp, self.template],axis=0),'label': label, 'name':name,'alpha_start': alpha_start} #, 'LQ_path': os.path.join(self.image_dir, 'blur', self.image_list[idx]), 'GT_path': os.path.join(self.image_dir, 'sharp', self.image_list[idx])
